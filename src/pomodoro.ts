...
export class PomodoroTimer {
  ...
  public start(minutes: number, onPhase: (phase: PomodoroPhase) => void): void {
    this.stop();
    this.remaining = minutes * 60;
    this.isBreak = false;

    const config = vscode.workspace.getConfiguration('hcf');
    const showStatus = config.get<boolean>('enableStatusBar', true);
    if (showStatus) { this.statusBarItem.show(); }

    this.timer = setInterval(() => {
      this.remaining--;
      const display = this.formatTime(this.remaining);
      const prefix = this.isBreak ? '☕ 休整' : '🎯 对焦';
      this.statusBarItem.text = `${prefix} ${display}`;

      if (this.remaining <= 0) {
        if (this.isBreak) { this.stop(); onPhase('break-end'); return; }
        this.completed++;
        onPhase('focus-end');
  ...
  public get completedCount(): number { return this.completed; }
  ...
  public dispose(): void { this.stop(); this.statusBarItem.dispose(); }
}
